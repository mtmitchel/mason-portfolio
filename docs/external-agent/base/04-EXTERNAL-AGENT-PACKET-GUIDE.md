...
packet is optional. Follow the folder lifecycle in
[`docs/external-agent-packets.md`](../../external-agent-packets.md) for naming,
refreshing live copies, separating returned output, and handling stale folders.
Do not add a manifest, ledger, receipt, or cleanup step merely to prove that the
folder was prepared.
...
Report local candidate checks, independent review status, and provider
synchronization separately. Do not infer one from another. A local edit does
not synchronize provider state.
