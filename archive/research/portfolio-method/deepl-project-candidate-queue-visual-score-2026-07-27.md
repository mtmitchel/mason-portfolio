# Archived DeepL project candidate queue

**Updated:** 2026-07-27
**Status:** Current discovery and ranking owner
**Source audit:** `/home/mason/Downloads/assistant-exports/chatgpt/deepl-project-candidate-audit.md`
**Current CV:** [`source-documents/mason-cv.pdf`](../../../private-evidence/source-documents/mason-cv.pdf)
**Figma source:** the complete 655-record ledger in
[`projects/pricing-evolution/figwright-raw-extractions.jsonl.gz`](../../../private-evidence/projects/pricing-evolution/figwright-raw-extractions.jsonl.gz)
...
First apply a viability floor. A candidate needs hiring-reader value, a
defensible account of Mason's contribution, and enough linked evidence for one
bounded story check. A Figma frame or mockup cannot make an otherwise weak or
unsupported candidate viable. Candidates with story readiness `0` remain in
reserve regardless of their visual score.

Score each candidate from `0` to `2` on reader value, story readiness, and
Figma-backed site-visual readiness:

- `2` means an exact or closely corresponding native Figma frame, mockup, or
  preserved Figma export can serve a clear evidence, explanation, or comparison
  role on the site and at least one known export is recruiter-ready or needs
  only a bounded crop, background, or annotation removal. A cleaner existing
  repository export counts when its Figma relationship is known.
- `1` means the relationship is contextual or adjacent, or the known exports
  need a clean re-export or substantial reconstruction before they can support
  the central story.
- `0` means no corresponding Figma visual is identified, or every known export
  is blank, incomplete, duplicated from another candidate, or unable to do a
  clear job in this candidate's story. A flattened chat attachment without a
  recoverable Figma frame can supplement later evidence, but it does not receive
  Figma-backed visual credit.

Story readiness `2` means there are enough linked sources for one bounded story
check; it does not mean the story is already verified. Visual readiness measures
both story usefulness and practical portfolio presentation. It does not select
an asset or prove contribution, lifecycle, shipment, or results.

For tied totals among viable candidates, prefer Figma-backed site-visual
readiness, then reader value, then story readiness, then the prior queue order.
New CV candidates follow CV order; new Figma candidates follow
extraction-inventory order. Existing public coverage and Mason's frozen work
...
| ---: | --- | --- | ---: | --- | --- | --- |
| 1 | `D06` | DeepL Write Pro launch and monetization work | `5` (`2/1/2`) | Full case | Active | Run one bounded story check linking the GPT evidence to Figma nodes `4:184736`, `4:186544`, `5:314534`, and `5:315615` plus the preserved Write Pro export. Confirm the selected visuals, Mason's contribution, lifecycle, supported ending, and material unknowns without reopening frozen Pricing. If the story remains strong, prepare the in-chat visual proposal required by the root workflow. |
| 2 | `C24` | DeepL Voice product content | `5` (`2/1/2`) | Mini case | Active | — |
| 3 | `C26` | Desktop app and integration discovery | `5` (`2/1/2`) | Mini or full case | Active | — |
| 4 | `C27` | Website navigation and discovery surfaces | `5` (`2/1/2`) | Mini or full case | Active | — |
| 5 | `D01` | Go Enterprise landing-page program | `5` (`2/2/1`) | Full case | Active | — |
| 6 | `D20` | Custom-domain account administration | `4` (`1/1/2`) | Mini case | Active | — |
| 7 | `D23` | Full-page translation as a paid feature | `4` (`1/1/2`) | Writing example | Active | — |
| 8 | `D10` | Plan changes and team-seat adjustment flows | `4` (`2/1/1`) | Mini case | Active | — |
| 9 | `D11` | Failed-payment and payment-method recovery | `4` (`2/1/1`) | Full or mini case | Active | — |
| 10 | `D17` | DeepL content style guide and naming principles | `4` (`2/1/1`) | Full or mini case | Active | — |
| 11 | `D03` | Apps and Integrations post-sign-up experiment | `4` (`2/2/0`) | Mini case | Active | — |
| 12 | `C25` | Mobile app and store-listing optimization | `4` (`2/2/0`) | Mini case | Active | — |
| 13 | `D13` | Glossary plan repackaging | `3` (`1/1/1`) | Writing example or mini case | Active | — |
| 14 | `D22` | DeepL Pro South Korea launch content | `3` (`1/1/1`) | Writing example | Active | — |
| 15 | `D09` | True-up billing UX | `3` (`2/1/0`) | Full or mini case | Active | — |
| 16 | `D18` | Figma content-review plugin prototype | `3` (`2/1/0`) | Mini case | Active | — |
| 17 | `C31` | Cross-channel marketing writing collection | `3` (`2/1/0`) | Writing collection | Active | — |
| 18 | `D02` | Industry landing-page localization and accessibility updates | `3` (`1/2/0`) | Mini case | Active | — |
| 19 | `D07` | Browser-extension SSO launch communication | `2` (`1/1/0`) | Writing example | Active | — |
| 20 | `D04` | Browser-extension ratings experiment | `2` (`1/1/0`) | Reserve | Active | — |
| 21 | `D05` | Firefox extension burger-menu experiment | `2` (`1/1/0`) | Reserve | Active | — |
| 22 | `F32` | Tier-aware cancellation, retention, and feature-loss copy | `4` (`2/0/2`) | Reserve mini case | Active | — |
| 23 | `F34` | Email verification and signup optimization | `4` (`2/0/2`) | Reserve mini case | Active | — |
| 24 | `F35` | Checkout abandonment and social-proof research | `4` (`2/0/2`) | Reserve mini case | Active | — |
| 25 | `F33` | US checkout consent | `3` (`1/0/2`) | Reserve writing example | Active | — |
| 26 | `F36` | API usage-limit UI | `3` (`1/0/2`) | Reserve writing example | Active | — |
| 27 | `D15` | Project Rebound | `2` (`2/0/0`) | Reserve | Active | — |
...
| — | `D21` | Localization research report and campaign | `4` (`2/2/0`) | Existing full case | Represented by the localization report campaign | — |
...
| — | `D14` | Pricing-grid readability and cumulative-tier exploration | `5` (`2/2/1`) | Frozen | Pricing Evolution remains frozen regardless of score | — |
...
Ten previous visual scores were too high:

- `D02`: `2` to `0` — no meaningful Figma match;
- `D03`: `1` to `0` — two flattened, near-duplicate sidebar PNGs survive in
  the chat export, but no native callout or sidebar frame is mapped;
- `D07`: `1` to `0` — the exported SSO cards are blank or incomplete and no
  polished exact artifact is mapped;
- `D09`: `2` to `0` — every export duplicates `D10` material and none contains
  true-up-specific language;
- `D10`: `2` to `1` — only two isolated modal interiors survive a tight crop;
  the surrounding flow uses placeholders, annotations, and working boards;
- `D11`: `2` to `1` — checkout, wallet, trust, and FAQ frames are adjacent to
  the recovery work rather than exact matches;
- `D13`: `2` to `1` — the exact audit inventory exists, but the raw spreadsheet
  and transparent rows need a portfolio-designed comparison treatment;
- `D14`: `2` to `1` — contextual Figma and flattened working mockups remain;
  the Pricing freeze is unchanged;
- `D18`: `1` to `0` — no plugin-project match exists in the product ledger;
- `D21`: `2` to `0` — no meaningful Figma match; its represented disposition
  is unchanged.

No candidate receives visual credit from a CV statement, text-only record,
assistant-generated description, or a frame that cannot do a clear job in the
candidate's story.
...
visuals. A further Figma export is justified only for `D10` or `D17`, and only
if that candidate advances. The other candidates need no Figma work, need a
bounded crop or site treatment, or lack a known exact surface and must not be
sent for speculative export.

| ID | Best current visual evidence | Portfolio disposition | Further Figma export |
| --- | --- | --- | --- |
| `D06` | `D06__feature-awareness-state-a__5-315616.png` through `state-d__5-315619.png`; the four Clarify popovers are useful supports | Ready. Prefer the existing public formality, glossary, and usage-limit images when one standalone upgrade prompt is enough. Exclude the feature board, banner board, and unfinished security cards. | None |
| `C24` | [`pricing-voice-clean.png`](../../../private-evidence/source-exports/figma/pricing-evolution/pricing-voice-clean.png); `C24__voice-header__5-308229.png` and `voice-faqs__5-308345.png` as supports | Use the existing clean repository frame. The ZIP's red working-copy pricing canvases, blank screenshots, and note frames are not portfolio assets. | None |
| `C26` | `C26__integration-zigzag__4-185814.png` | Ready without cleanup. Reject the full-page WIP and cards with grey direction placeholders. | None; extract the current PNG from the ZIP only if selected |
| `C27` | `C27__navigation-variant-a__2-16920.png` and `navigation-variant-e__2-16937.png` | Promising pair after a tight crop to the inner product viewport. Exclude the grey canvas, Anonymous label, empty states, clipped dropdown, and eight-state board. | None; crop the current exports |
| `D01` | Existing clean API, Write add-on, Voice, and bundle-era repository exports | Visually stronger than the ZIP, but still contextual to the enterprise-program story. The ZIP pricing grids contain red working copy, placeholders, or unavailable-component warnings. Pricing remains frozen. | None for visual quality; do not request a speculative enterprise export |
| `D20` | Existing curated files under [`custom-domain/`](../../../private-evidence/source-exports/figma/team-administration/custom-domain/) | Prefer the existing neutral `acme` dialogs and full-page states. The ZIP emails can support the ending after a tight crop. Exclude journey cards, blank references, Phoenix tables, Slack mockups, and uncropped 9,508-pixel settings pages. | None |
| `D23` | [`pricing-translator-transition-table.png`](../../../private-evidence/source-exports/figma/pricing-evolution/site-display/pricing-translator-transition-table.png) | Use a purpose-built crop preserving the plan labels and “Full-page translation” row. Reject the raw full-page ZIP canvas. This does not reopen the frozen Pricing story. | None; crop the existing clean table |
| `D10` | `D10__downgrade-single-b__2-19002.png` and `downgrade-bundle-c__2-19117.png` | Salvage only the centered modal interiors. There is no recruiter-ready full frame; exclude placeholder screens and composite boards. | Conditional: find and export clean full-flow states only if this candidate advances |
| `D11` | `D11__faq-translator-collapsed__3793-16082.png` and `faq-translator-expanded__3793-16127.png` | The pair is polished, but it proves the FAQ intervention, not failed-payment recovery. Keep component close-ups on a consistent light background. | None; this is a story-fit gap, not an export gap |
| `D17` | `D17__suggestions-comparison__2-20734.png`; existing clean upgrade-prompt exports | Usable support only. Naming boards contain excess canvas, pen marks, annotations, or WIP structure and need a clean re-export or site-native reconstruction before use. | Conditional: clean naming-comparison frame only if this candidate advances |
| `D13` | `D13__tooltip-inventory__5-313460.png` | Source evidence, not a raw site image. Turn selected original/revision rows into labeled before-and-after comparisons; do not publish the full spreadsheet or transparent row strips. | None; this needs site design, not another export |
| `D22` | `D22__doc-translation-active__5-313946.png` and `file-translation-korean__5-313911.png` | Polished contextual cards, but neither proves the South Korea launch, retained launch copy, or performance. Exclude component canvases and DM-2113 boards with unavailable-component warnings. | None; this is an evidence gap, not an export gap |
| `D09` | None | Reject the set. It duplicates `D10` and contains no true-up-specific visual. | None; no exact true-up surface is known |
| `D07` | None | Reject the set. Three exports are isolated transparent text fragments; the fourth contains a blank placeholder. | None; no finished exact surface is known |

Existing clean Pricing-derived assets above are reuse candidates only. Their
presence does not authorize Pricing narrative work, route changes, asset
replacement, or publication.
...
| `D03` | Row 3; `2eca15ca…`, `0f56d90a…`, `f7ef4aa7…` | No native match in the 655-record ledger. Two near-duplicate flattened sidebar canvases survive as `/home/mason/Downloads/assistant-exports/chatgpt/file-xNKo6fnadHuNem5TegfqCsuu.dat` (`1582 × 1608`) and `/home/mason/Downloads/assistant-exports/chatgpt/file-8SHcT3DWrCW0IxErbHvSfINL.dat` (`1606 × 1644`); neither is the original callout or a mapped, editable Figma frame. | `P1`, `P2`, `P5` | `APP-01`. Mason wrote or refined copy and a follow-up says the hypothesis was proven; no numeric lift or sidebar lifecycle is established. | Reinforces with a native-Figma gap and flattened sidebar context |
...
| `D06` | Row 6; `f3072da5…`, `e5b7dc87…`, `c01a612b…`, `6737306d…` | Correlated: `Monetization` / `Write Pro` (`4:181271`) / nodes `4:184736`, `4:186544`; plus `Pricing page` (`3:94360`) / nodes `5:314534`, `5:315615`. The complete `5:315615` feature-awareness frame is preserved at [`source-exports/figma/write-pro-launch/feature-awareness-security-2x.png`](../../../private-evidence/source-exports/figma/write-pro-launch/feature-awareness-security-2x.png). | `P1`, `P2`, `P5` | `MON-05`, `SYS-04`. Collaborative conception, writing, direction, editing, selection, and application are supported; exact variants, sole launch ownership, and conversion effect are not. | Reinforces |
