...
decisions. The current public rewrite below supersedes older implementation
history later in this file wherever they conflict. Current disk evidence and
the private asset manifest remain the source of truth for the actual
implementation and source details.
...
reduced to six artifacts. Upgrade prompts and pricing are separate stories.
Account recovery and team administration form one case with two focused
mini-stories. The homepage uses a row-major grid, so visual, DOM, keyboard and
screen-reader order all follow this list.
...
The page opens with: “Five published examples show how I change structure,
tone, and level of detail for product announcements, methodology, instruction,
data-led press releases, and consumer writing.” Each entry keeps its source
text selectable, puts Mason's first-person writing decision beside the sample,
and uses publication context only when the live page could create a misleading
impression.
...
## Current public case structures

Every case uses its own story order. Shared components may control presentation,
but no shared data shape may force Problem, Constraint, Decisions, Trade-off,
Outcome or Evidence-note sections onto every page.
...
The 12% paid-conversion lift and seven-figure ARR increase are stated once and
belong to the wider experiment wave, not to one prompt.
...
switcher. The route contains no conversion or revenue result.
...
The crop-led story comes first. The accessible switcher follows it and keeps
the four complete frames available without placing them in one long stack.

### Account recovery and team administration

1. Separate an incorrect code from lost access.
2. Explain what the reset changes.
3. End on the visible MFA active state.
4. Name the consequence before deleting users.
...
1. Build the report around marketers' questions.
2. Change the entry point, keep the figures.
3. Turn webinar material into operating stories.
4. Published work and campaign result.

The six-figure lead-generation pipeline belongs to the campaign. Mason's
contribution is writing, editing, structure and repurposing across the formats.

### Executive ghostwriting on global mobility

The opener names Hanna Marie Asmussen, says Mason wrote the articles on her
behalf, and says Forbes published them under Hanna's byline. Three sequential
article units show one measured first-person voice serving three different
arguments. The case page uses selectable published text and no artwork.

## Public writing and caption rules

- Open with the full story promised by the page title.
- State Mason's role in normal prose.
- Put each content claim beside the image or excerpt that supports it.
- Use before and after only for two real states.
- Keep one idea per paragraph.
- Captions tell the reader what to notice about the content decision. They do
  not discuss files, source handling, publication status or private review.
- Unsupported claims are omitted. The public page does not display a disclaimer
  about what cannot be proved.
- Figma variables such as `{5}`, `{1}` and `{16}` stay in private
  documentation. Public copy explains their meaning without showing the tokens.

The current homepage cover for Localyze is the square passport animation and
its poster. Keep the loop; the older archive-thumbnail direction is retired.

## Private source and implementation history

The notes below retain source history and private implementation context. They
do not define the current public story order.
...
- `{5}`, `{1}` and `{16}` in the document frames are design-file variables that
  are replaced with user-specific values. They remain private documentation,
  not public caption text.
...
- **Exhibit consistency rule (rev 3, 2026-07-23):** the four product-page
  ...
  rebuilds from `private-evidence/source-exports/figma/monetization-pricing-pages/API.png`; discounts follow the discount
...
- **Bundle-era history correction (Mason, 2026-07-24):** the shipped bundle
  ...
  `private-evidence/source-exports/figma/pricing-page-screenshots/1920px.png`, `1440px.png` and the research's June 2024
...
- Provenance (updated 2026-07-23): the full bundle-era pricing pages
  ...
  Mason's own 2x exports live in `private-evidence/source-exports/figma/monetization-pricing-pages/` at the repository root
...
- **Price display (revised by Mason, 2026-07-24):** every exhibit shows its
  ...
  `private-evidence/source-exports/figma/monetization-pricing-pages/Translator@2x.png`); Write Free €0, Write Pro €10 per
  user/month billed annually, Write for Business "Let's talk"
  (`private-evidence/source-exports/figma/monetization-pricing-pages/Write.png`, toggle `Save 33% with annual`); API Free €0,
  API Pro €4.99 "per month + usage-based price" (`private-evidence/source-exports/figma/monetization-pricing-pages/API.png`
  ...
  on the API Free card in `private-evidence/source-exports/figma/monetization-pricing-pages/API.png` is a placeholder error —
...
- Figma execution instructions live in
  `archive/research/pricing-evolution/figma-exhibit-brief.md` (single execution owner
  for all exhibit edits and exports; the older Chrome-profile export queue is
  retired). Exports land in
  `private-evidence/source-exports/figma/pricing-evolution/`; Mason approves them
...
- **Comparison tables:** only two exist in the sources — on the bundle full
  ...
  API source pages have none (verified against `private-evidence/source-exports/figma/monetization-pricing-pages/Write.png`,
...
- **Full pages are not shipped to the site.** The bundle page (3024x15231)
  ...
  2026-07-24, in `private-evidence/source-exports/figma/pricing-evolution/
...
- Keep the square passport animation and its poster on the homepage only. The
  case page is led by the published writing, not artwork.
