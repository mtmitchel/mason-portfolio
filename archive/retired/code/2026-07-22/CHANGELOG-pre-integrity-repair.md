...
Status: the five-project and writing-index revision is implemented and validated
locally. It is not committed, pushed or published.

### Added

- Route-backed `Selected work` and `Writing` indexes with a shared header, About
  section and footer; the writing view remains directly linkable at `/writing`
  without client-side tab state.
- Artifact-led SmartRecruiters Hiring Success and Localyze executive-ghostwriting
  mini cases using public historical work and conservative contribution claims.
- A readable writing collection spanning DeepL, Localyze, SmartRecruiters,
  Joblift and Kitchen Stories.
- A private selected-asset manifest recording provenance, permission, privacy,
  crop, caption and text-alternative decisions.
...
- An earlier private candidate-review route and exhaustive Team Administration
  galleries used to make the final selection. Those routes now redirect and the
  source galleries are preserved outside the deployable public tree.
...
- Reduced the main index to five substantial project stories and replaced the
  proposed collection tile with quiet `Selected work` and `Writing` links.
- Merged the localization report and campaign into one excerpt-led case with
  selectable prose, an adaptation sequence and live published-work links.
- Removed decorative section numbering, repeated stakes headings and templated
  evidence closers from the retained DeepL cases; case-specific headings now
  follow the evidence.
- Retargeted all retained case navigation and retired aliases so no route points
  back into the candidate-review surface or creates a redirect chain.
- Relocated assets used only by retired routes into ignored private evidence,
  preserving provenance while removing them from the deployable asset tree.
...
- The candidate-review index, unselected case pages and exhaustive pre-selection
  galleries from the public route and asset surface. Historical sources remain
  preserved privately.
- The orphaned UX-copy page; its URL now leads to the writing index.
...
- All rendered-route and asset tests passed.
- Selected DeepL, Localyze, SmartRecruiters, Forbes, Joblift and Kitchen Stories
  publication links resolved through live-page or direct-response checks.
- All 150 retired public assets were matched byte-for-byte to their preserved
  private originals.
...
- The candidate-review route is retired and redirects to the selected-work index.
- The current revision, Claude/Fable handover and PDF review assets are not
  committed, and no branch has been pushed to GitHub.
...
- Publication remains gated on Mason's factual, ownership, confidentiality and
  employment-agreement review, followed by explicit approval for the remaining
  commit, push, public access and deployment.
