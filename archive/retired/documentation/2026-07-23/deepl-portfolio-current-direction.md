...
- The 12% paid-conversion lift and seven-figure ARR uplift belong to the whole
  experiment wave. Never attribute either result to one modal, screen or copy
  change.
...
- `Terms to never edit` is a real Write feature (checkmarked per plan in the
  bundle-era comparison table), not a placeholder. Public copy uses the
  polished form `Set terms to never edit`.
...
- **Reconstruction labeling rule:** the five exhibits (bundle-era page,
  cumulative Translator grid, Write add-on page, Voice page, API page) are
  refined recreations built in duplicated frames in the Pricing Grid file
  (`fdxAxs14RUKijGsznWusF3`). Public captions present them as refined from
  the historical working files — never as pixel-exact shipped UI — and no
  metric is attached to them.
- **Exhibit consistency rule (rev 3, 2026-07-23):** the four product-page
  ...
  drafts; Write Pro lists tighten to five lines; the bundle toggle label
  completes as `Save 16% with annual`. The old-design `Translate Tab` /
...
- Provenance (updated 2026-07-23): the full bundle-era pricing pages
  ...
  red styling.
- **No-causal-credit ceiling:** DeepL's current public pricing page uses a
  cumulative "Everything in X, plus" tier pattern. The team's improvement
  board documents a simplification backlog ("Reduce number of features in
  pricing grid" rated high impact / low effort, "Comparison table change",
  "Unified pricing page") but does not name the cumulative pattern and does
  not attribute individual notes to Mason. Public wording may say the
  reconstruction "extends the simplification direction the team had
  prioritized" and may show the current public page as a reference. It must
  never claim Mason's suggestion caused DeepL's current design.
- The improvement board (exported as `private-evidence/source-exports/figma/pricing-page-screenshots/Section
  2085662595.*`) reads as internal tickets and stays private. It supports at
  most one narrative sentence about the documented simplification backlog.
...
- The bundle-era comparison table appears once, unmodified, as density
  evidence. No table redesign.
- Figma execution instructions live in
  `archive/research/pricing-evolution/figma-exhibit-brief.md` (Codex, via Mason's
...
- The launch was iterative and imperfect. The artifacts support visible design
  states and Mason's contribution, not final approval, shipment or performance.
- Never attach the upgrade-wave 12% conversion or seven-figure ARR result to
  pricing, feature awareness or an individual launch frame.
...
`organization`; make no other historical design change. Historical prices and
plan details may be shown only as period-specific launch evidence, never as
...
- Do not add shipment, adoption, reduced-error, support-volume or business
  outcome claims. MFA evidence must never substantiate the separate SSO claim.
...
- The artifact supports the conditional content model across product, price,
  seats, eligibility, timing and consent. No conversion result is attached to
  this case.
- Trial-eligibility and global-pricing aliases redirect directly to checkout.

## Editorial-case claim ceilings
...
- Mason's supported contribution is writing, editing, structuring and
  repurposing across the formats. No campaign-performance metric is claimed.
...
- The public byline belongs to Hanna Marie Asmussen. Do not present Mason as the
  public bylined author.
...
The site remains local, uncommitted and unpublished. Figma remains read-only.
