# Current portfolio direction

**Updated:** 2026-07-24
**Status:** Current decision owner

This file records the current public structure. The
[full previous direction](../archive/research/deepl-portfolio-direction-full-2026-07-24.md)
is preserved for history and does not override this file.

## Homepage and About

The homepage contains six cards in this order:

1. **Upgrade prompts across Translator and Write** — `/work/upgrade-prompts`
2. **Simplifying pricing across four products** — `/work/pricing-evolution`
3. **Checkout across four purchase states** — `/work/checkout`
4. **Account recovery and team administration** — `/work/account-team-security`
5. **Turning localization research into a multi-format campaign** — `/work/report-campaign`
6. **Executive ghostwriting on global mobility** — `/work/localyze-executive-ghostwriting`

Keep the restrained three-column grid, the Selected work and Writing links, the
short role line, and the current metadata placement and size. Do not add tags,
summaries, badges, or another filter.

The pricing, account-security, and Localyze cards keep their silent loops. The
localization-report card shows the complete `978 × 1369` report cover at its
natural proportions. It must never be cropped.

About stays at two short paragraphs:

1. Mason writes explanations, limits, prices, and confirmations so people
   understand what happened, what to do next, and what a decision means.
2. His DeepL work spans Translator, Write, subscriptions, pricing, and
   enterprise accounts; earlier product and editorial work includes Localyze,
   SmartRecruiters, Joblift, and Kitchen Stories.

## Rules for every case

- Open with the complete story promised by the title.
- State Mason's role in normal prose.
- Name visible problems instead of using generic problem statements.
- Put each claim beside the image or excerpt that supports it.
- Use before and after only for two real states.
- Keep one idea per paragraph.
- Use captions to tell the reader what to notice.
- Omit unsupported claims without displaying a disclaimer.
- Keep source handling, crop notes, Figma variables, claim review, and
  publication status private.
- Let each case use its own story shape. Shared code handles presentation, not
  narrative order.

## Case structures

### Upgrade prompts across Translator and Write

1. Different interruptions needed different explanations.
2. Name why the task stopped.
3. Match the paid value to the task.
4. Change the offer for the account state.
5. Wave result.

The 12% paid-conversion lift and seven-figure ARR increase appear once and
belong to the wider experiment wave.

### Simplifying pricing across four products

1. When there was one product to buy.
2. A second paid product opened three ways to buy.
3. Two problems inside every bundle card.
4. Put the add-on choice on the Translator page.
5. Replace repetition with progression.
6. Move qualifiers into row labels.
7. Three products the ladder did not fit.
8. One page per product, one job for each line.

The case opens on the single-product condition so the complication has
something to complicate, then names the three buying routes, the content those
routes produced, the add-on pivot, the cumulative rewrite, the table rewrite,
and the three products the cumulative ladder did not fit. Repetition to
progression is the only Before-and-After comparison; every other pair is a
neutral two-state switcher. Each evidence image is one intentional crop, and
the dialog opens that same crop. Paired evidence shares a recorded crop scope,
the cumulative comparison shows every tier in its grid, and the three remaining
product states use one consistent card-row crop. Translator's grid appears once,
in the repetition-to-progression comparison, and is deliberately not shown
again in the final chapter. Every crop is trimmed flush to its own content and
the page frame supplies the surrounding space, so no image carries baked-in
padding or dead margin. Chapter copy, captions, alternative text, and
claim limits are owned by
[`pricing-evolution-story-spec.md`](pricing-evolution-story-spec.md). The case
contains no conversion or revenue claim.

### Checkout across four purchase states

1. Carry the trial into account creation.
2. Keep a two-product offer visible at commitment.
3. Scale the order to a team.
4. Change timing and consent when no trial applies.
5. Explore the complete purchase states.
6. End with observable differences.

The crop-led story comes first. The accessible switcher follows it.

### Account recovery and team administration

1. Separate an incorrect code from lost access.
2. Explain what the reset changes.
3. End on the visible MFA active state.
4. Name the consequence before deleting users.
5. Confirm the filtered result.

The page makes only observable claims about the selected states.

### Turning localization research into a multi-format campaign

1. Build the report around marketers' questions.
2. Change the entry point, keep the figures.
3. Turn webinar material into operating stories.
4. Show published work and the campaign result.

The six-figure lead-generation pipeline belongs to the complete campaign.
Mason's contribution is writing, editing, structure, and repurposing.

### Executive ghostwriting on global mobility

The opener names Localyze co-founder and CEO Hanna Marie Asmussen, says Mason
wrote the articles on her behalf, and says Forbes published them under Hanna's
byline. Three article units show one measured first-person voice serving three
different arguments. The case page uses selectable text and no artwork.

## Writing collection

`/writing` contains five entries in this order:

1. Localyze 2.0 announcement and beginner's guide
2. SmartRecruiters' *The Definitive Guide to Hiring Success*
3. SmartRecruiters onboarding guide
4. Joblift renewable-energy press release
5. Kitchen Stories coq au vin editorial

Each entry contains employer and format, title, one short context sentence,
exact selectable samples, a first-person explanation of the writing decision,
and a public link. Publication context appears only when the live source could
mislead the reader about Mason's original contribution.

## Evidence and publication

The selected-asset source and crop record is
[`portfolio-asset-manifest.json`](portfolio-asset-manifest.json). Current claim
limits are in [`claim-review.md`](claim-review.md).

The repository and site remain private. Commit and push do not authorize public
access or deployment.
